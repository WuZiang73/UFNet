...
from skimage.metrics import peak_signal_noise_ratio as compare_psnr #tcw201904101622tcw
...
def parse_args():
    ...
    parser.add_argument("--test_data_dir", type=str, default="datasets/Urban100")
...
def sample(net, device, dataset, cfg):
    ...
    for step, (hr, lr, name) in tqdm(enumerate(dataset)):
        if "DIV2K" in dataset.name:
            ...
            lr = add_noise(lr,1,25).to(device)
            sr = net(cfg.scale, lr)[0].detach().squeeze(0) #detach() break the reversed transformation.
...
def main(cfg):
    ...
    device = torch.device("cuda:0") #0 is number of gpu, if this gpu1 is work, you can set it into 1 (device=torch.device("cuda:1" if torch.cuda.is_available() else "cpu"))
    net = net.to(device)
