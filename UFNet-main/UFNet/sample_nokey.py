...
def parse_args():
    ...
    parser.add_argument("--ckpt_path", type=str,
                        ...
                        default="DSRNet-main/checkpoint/DSRNet_perceptual/DSRNet_x4_600000.pth")  # Replace with your setting.
    ...
    parser.add_argument("--loss_fn", type=str, default="Charbonnier", choices=["MSE", "L1", "SmoothL1","Charbonnier","Perceptual"])
...
def sample(net, device, dataset, cfg):
    ...
    for step, (hr, lr, name) in enumerate(dataset):
        if "DIV2K1" in dataset.name:
            ...
            # 如果没有特殊噪声处理，lr_noise可以直接用lr；否则用你的add_noise函数
            lr_noise = lr  # 或 lr_noise = add_noise(lr, mode, noise_level)
            output_tuple = net(cfg.scale, lr_noise)
            sr = output_tuple[1].detach().squeeze(0)
            t2 = time.time()
        ...
        bnd = scale
        sr_1 = rgb2ycbcr(sr, only_y=True)
        hr_1 = rgb2ycbcr(hr, only_y=True)
...
def main(cfg):
    ...
    state = torch.load(cfg.ckpt_path)
